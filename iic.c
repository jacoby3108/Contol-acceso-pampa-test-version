...
void i2c_trans(u8 size, u8 id, u8 *buffer, u16 address, u8 mode) {
    ...
    ready=0;                // This must be done before I2C interrupts are enabled
    ...
    i2c_data.mode = mode;
    UCB0I2CSA = id >> 1; //Slave Address 01010000 0x50 EEPROM (slave address is 7 bits 1010 A2 A1 A0
...
void dummy_read(void)
{
    ...
    IE2 &= UCB0TXIE ; //Enable TX, RX interrupt
...
// ======================================== WRITE FSM ========================================
	...
	if (i2c_data.mode == WRITE_M)			// Write mode state machine
	{
		...
		return 0; /// continue sending the frame

	} //End Write mode state machine
	...
	if (i2c_data.mode == READ_M)				//Read mode state machine
	{
		...
		else if (i2c_data.state == RX_DATA)		// Last repeated start TX Interrupt
		{
			...
			*(i2c_data.buffer)++= UCB0RXBUF;	// And store Data
			...
			if(i2c_data.count==2 ||i2c_data.count==1  ) // No more Data will be needed
			{
               #if DEBUG_I2C == TRUE
			   P1OUT |= GREEN;  // just for debug
               #endif

			  //Note: In master receiver mode the STOP condition is preceded by a NACK by hardware.
			  //See User manual page 469
					   ...
					   if(i2c_data.count==2)
					       *(i2c_data.buffer)++= UCB0RXBUF;  // Save last data. Reading UCBxRXBUF resets UCBxRXIFG
					   if(i2c_data.count==1)
					       temp= UCB0RXBUF;                  // Discard last data
